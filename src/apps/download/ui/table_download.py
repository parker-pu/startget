...
"""
from PySide6.QtWidgets import QTableWidget, QAbstractItemView, QTableWidgetItem, QProgressBar

from src.utils.file.load import Load


class TableDownload(QTableWidget):
    parent = None
    ...
    def table_style(self):
        ...
        # 设置水平表头标签
        self.setHorizontalHeaderLabels(["File Name", "Progress", "Status", "Other"])
    ...
    def load_table(self):
        ...
        for row in hd:
            ...
            ''')
            progress_bar.setValue(int(row.get("progress") * 100))
            self.setCellWidget(index, 1, progress_bar)
            self.setItem(index, 2, QTableWidgetItem(str(row.get("Status"))))
            self.setItem(index, 3, QTableWidgetItem(row.get("Last Modified")))
